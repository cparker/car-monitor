...
class MPU6050:
   ...
   #AccelerationFactor= 16.0/32768.0;   #assuming +/- 16G
   AccelerationFactor= 2.0/32768.0    #assuming +/- 2G
   ...
   def DoUnitsConversion(self,ShortData):
      #lets create the Data Class
      AccData = MPU6050Data()

      # raw
      AccData.rawGx = ShortData[0]
      AccData.rawGy = ShortData[1]
      AccData.rawGz = ShortData[2]
      ...
      #and the 3 last are the gyro data

      AccData.Gyrox = ShortData[4] * self.GyroFactor
      AccData.Gyrox = ShortData[5] * self.GyroFactor
      AccData.Gyrox = ShortData[6] * self.GyroFactor

      return AccData


   def GotNewData(self):
      #check if interrupt flag to tell us if we have new data
      statusFlag = bus.read_byte_data(self.MPU6050_ADDRESS,self.MPU6050_RA_INT_STATUS)
      return ((statusFlag & 0x1) ==1)

   def Setup(self):

      #set sample rate to 100Hz (8000/100)=80 => 79
      bus.write_byte_data(self.MPU6050_ADDRESS,self.MPU6050_RA_SMPLRT_DIV,79)
      ...
      #bus.write_byte_data(self.MPU6050_ADDRESS,self.MPU6050_RA_ACCEL_CONFIG, 0x18)
      bus.write_byte_data(self.MPU6050_ADDRESS,self.MPU6050_RA_ACCEL_CONFIG, 0x00)
   ...
   def ReadData(self):
      ...
      #convert list of 14 values bytes into 7 values short using the unpack method
      ShortData = struct.unpack(">hhhhhhh",buffer(bytearray(GData)))
      #convert raw data into units. G for accelerometer , Celsius and degree / sec for gyro
      return self.DoUnitsConversion(ShortData)